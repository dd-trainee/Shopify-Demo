...
slate.Variants = (function() {
  ...
  function Variants(options) {
    ...
    //custome filter image function
    this.filterImage(this.currentVariant);
  ...
  Variants.prototype = Object.assign({}, Variants.prototype, {
    ...
    /*Product thumbnail image change on differnt variants selected..
      only works when product featured image and image alt text has exsist */
    
      filterImage: function(variant)
      {
        
        //var s=variant.option1;
        //console.log(s);
        //console.log(altLowerCase);
        
        //var altLowerCase= variant.featured_image.alt.toLowerCase();
        //var pTitle=$('.product-single__title').html().toLowerCase().trim();
		//console.log(pTitle);
        if(variant.featured_image != null && variant.featured_image.alt != null )   
        {
          var altLowerCase= variant.featured_image.alt.toLowerCase();
          $('[data-thumbnail-value]').hide();

          var selected_value = altLowerCase;
          var thumbnail_select= '[data-thumbnail-value="' + selected_value + '"]';

          console.log(thumbnail_select);
          
          $(thumbnail_select).show();
        }
        
        else
         {
           $('[data-thumbnail-value]').show();
	  ...
	  if (Shopify.queue.length) {
	    ...
	    $.ajax({
          ...
          success: function(res){
	        Shopify.moveAlong();
            var img=res.featured_image.url;
            //console.log(img)
            k+=1;
            //console.log(res);
            //console.log(k)
            //console.log(newArray.length-1)
            $('.col-modal-body').html('<img class="col-img" src="'+img+'">');
            $('.col-text').html('<b>'+k+'</b>'+" Product Added Out Of " +'<b>'+(newArray.length-1)+'</b>');
...
//View More.. button on collection page for collection description.
if (location.href.indexOf("collections/") > -1)
{
  
 if($('.collection-description').text().trim().length < 80)
 { 
   $('.loadMore').hide()  
 }
  ...
  {
    var h = $('.collection-description')[0].scrollHeight;
...
//load more button to LOAD more product

var product=$('.product-per-page');
var next_page_url=$('.product-per-page').data('next-url');
var product_show=$('.product-per-page').attr('product-remain');
var total_product=$('.product-per-page').attr('total-product');
var load_more_btn=$('.load-more__btn');
var load_more_spinner=$('.load-more__spinner');

var quick_view_btn=$('.quick-view-button');
...
function LoadMoreProducts()
{
          ...
          var new_product=$(next_page).find('.product-per-page'); 
          var new_url=new_product.data('next-url');
             
          var pro_remain=new_product.attr('product-remain');
          var pro_count=parseInt(product_show) + parseInt(pro_remain);
            
            //console.log(total_product);
            if(pro_count >= parseInt(total_product))
            {	
              $('.product-remaining').html("Great!! You Visited" + " " + '<b>'+total_product+'</b>');
              $('#progressBar').val(total_product);
             
           	}
            else
            {
              $('.product-remaining').html("You Visited" + " " + '<b>'+pro_count+'</b>');
              $('#progressBar').val(pro_count);
            }  
           if(new_url)
           {
             load_more_btn.show();
          ...
          product_show=pro_count;
          next_page_url=new_url;
          product.append(new_product.html());
          
          /*Quick View Function calling*/
          quickView();
...
//Product filter(throgh product tag) in collection page via ajax

var t=$('.product-per-page');
var spinner=$('.filter-spinner');
var load_btn=$('.filterBtn');

$('select.customTag').change(function(){
load_more_btn.hide();  
t.empty();
spinner.show();  
var s=$('.customTag option:selected').attr('value')
//console.log(s);
          $.ajax({   
          url:s,  
          type:'GET',  
        ...
        spinner.hide();    
		var ss=$(data).find('.product-per-page');       
        var find_url=ss.data('next-url');
        ...
        { 
        	load_btn.show();
             ...
             $.ajax({   
              ...
              var c=$(data).find('.product-per-page');
              var cc=c.data('next-url');
              //console.log(cc);
...
//product Sorting (through product SORT TAG) in collection page.

var a=$('.product-per-page');

$('.customSort').change(function sorting(){
var aa=$('.customTag option:selected').attr('value')  
a.empty();  
spinner.show();  
var s= aa + '?sort_by=' + $('.customSort option:selected').attr('value')
console.log(s);
$.ajax({
    url:s,
    type:'GET',
...
//collection slider in home page

$(document).ready(function(){
 if( $('.row-custom-card').length > 0 )
 	{
      $('.row-custom-card').slick({
        centerMode: true,
        centerPadding: '60px',
        speed: 500,
        autoplaySpeed: 2000,
...
//             html += '<tr data-varid="'+item.id+'" line-item="'+td+'">';
//                 html += '<td><img src="'+item.image+'" alt="'+item.product_title+'" style="height:50px;"></td>';
//                 html += '<td>'+item.product_title+'</td>';
//                 html += '<td>'+theme.Currency.formatMoney(item.price, theme.moneyFormat)+'</td>';
//                 html += '<td><input type="number" value="'+item.quantity+'" class="qty-update"></td>';
//                 html += '<td>'+theme.Currency.formatMoney(item.line_price, theme.moneyFormat)+ '</td>';
//                 html += '<td><a href="javascript:void(0);" onclick="removeCart();" class="cart-remove">Remove</a></td>';;
//             html += '</tr>';
...
//      var qty = $(this).val(),
//          variant_id = $(this).closest('tr').attr( "data-varid" );
//      $.ajax({
//        type: 'POST',
//        url: '/cart/change.js',
//        dataType: 'json',
...
//        var qty = 0,
//         line = $(this).closest('tr').attr( "line-item" );
//       //console.log(line)
//      $.ajax({
//        type: 'POST',
//        url: '/cart/change.js',
